...
class GmailEmailer:
    ...
    def send_cold_emails(self):
        ...
        try:
            ...
            print(f"[ERROR] Could not read recipients.txt: {e}")
            recipients = []
        # Read template
        try:
            with open('cold_email.txt', 'r') as f:
                template = f.read()
        except Exception as e:
            print(f"[ERROR] Could not read cold_email.txt: {e}")
            template = "Hello {{name}},\n\nI'd love to connect with you.\n\nBest, {{your_name}}"
        subject = "Let's connect!"
        for to_email in recipients:
            name = to_email.split('@')[0]
            body = template.replace('{{name}}', name).replace('{{your_name}}', 'Your Name')
            ...
            print(f"[INFO] Sent cold email to {to_email}")
